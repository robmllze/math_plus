...
num example_sin(final double a, [final int accuracy = 10]) {
  ...
  final _denomenators =
      List<num>.generate(accuracy, (n) => factorial(2 * n + 1));
  return _numerators.quotient2(_denomenators).altering1();
}

num example_sec(final double a, [final int accuracy = 10]) =>
    1 / example_cos(a);
...
num example_csc(final double a, [final int accuracy = 10]) =>
    example_sec(piHalf - a);

num example_cot(final double a, [final int accuracy = 10]) =>
    example_tan(piHalf - a);
...
num example_asin(final double a, [final int accuracy = 10]) {
  ...
  return List<num>.generate(accuracy, (n) {
    final c = 2 * n;
    final d = c + 1;
    return factorialT100(c) /
        (pow(4, n) * pow(factorialT100(n), 2) * d) *
        pow(a, d);
  }).sum();
}

num example_acos(final double a, [final int accuracy = 10]) =>
    piHalf - example_asin(a, accuracy);
